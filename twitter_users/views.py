...
def twitter_login(request, redirect_field_name='next'):
    ...
    return HttpResponseRedirect(request_token.authenticate_url)
...
def twitter_callback(request):
    ...
    #
    user = authenticate(username=token['username'])

    #if user not exist create it
    if not user:
        user = User(twitter_id  = token['user_id'],
                            username    = token['username'],
                            token       = token['token'],
                            secret      = token['secret'])
        user.save()
        #need this for necesary fields
        user = authenticate(username=token['username'])
